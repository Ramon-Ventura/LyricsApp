...
class ArtistSongActivity : AppCompatActivity() {
    ...
    private fun getUrlApi(artist : String, track : String) : String{
        return "https://orion.apiseeds.com/api/music/lyric/$artist/$track?apikey=0Jkf0hYwg0vpSWe299SHccDA4XOChzrwXPn6NArCnje5vNsuz3k8IzBNQCnCwqJs"
    }
    ...
    private fun jsonToObject(response: String) {
        ...
        if(apiResponse.result!=null){
            ...
            intent.putExtra("Lyric",apiResponse.result?.track?.text)
            intent.putExtra("Artist",apiResponse.result?.artist?.name)
            intent.putExtra("Song",apiResponse.result?.track?.name)
            progressDialog.dismiss()
    ...
    private fun showDialogEmpty(){
        ...
        alert.text = "Artist or Song value is missing, be sure to insert the necessary data"
    ...
    private fun showDialogNull(){
        ...
        val alert = TextView(this)
        alert.setTextSize(TypedValue.COMPLEX_UNIT_SP,20.0f)
        alert.text = "We didn't find a lyric for that song and artist"
        linearLayout.addView(alert)
        linearLayout.setPadding(10,10,10,10)
        builder.setView(linearLayout)
